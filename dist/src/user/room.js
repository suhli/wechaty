...
exports.wechatifyRoom = exports.Room = void 0;
/**
 ...
 *
 */
class Room extends room_events_1.RoomEventEmitter {
    /**
     * @hideconstructor
    ...
    static async findAll(query) {
        ...
        try {
            ...
            while (batchIndex * BATCH_SIZE < roomList.length) {
                const batchRoomList = roomList.slice(BATCH_SIZE * batchIndex, BATCH_SIZE * (batchIndex + 1));
                await Promise.all(batchRoomList.map(room => room.ready()
                    .catch(e => {
                    config_1.log.warn('Room', 'findAll() room.ready() rejection:\n%s', e.stack);
    ...
    async ready(forceSync = false) {
        ...
        if (forceSync) {
            await this.wechaty.puppet.dirtyPayload(wechaty_puppet_1.PayloadType.Room, this.id);
            await this.wechaty.puppet.dirtyPayload(wechaty_puppet_1.PayloadType.RoomMember, this.id);
        }
        ...
        while (batchIndex * BATCH_SIZE < memberList.length) {
            const batchMemberList = memberList.slice(BATCH_SIZE * batchIndex, BATCH_SIZE * (batchIndex + 1));
            await Promise.all(batchMemberList.map(c => c.ready().catch(e => config_1.log.error('Room', 'ready() member.ready() exception:\n%s', e.stack))));
            batchIndex++;
        }
    }
    /**
    ...
    async say(something, ...varList) {
        ...
        function isTemplateStringArray(tsa) {
        ...
        if (isTemplateStringArray(something)) {
            const msgId = await this.sayTemplateStringsArray(something, ...varList);
            if (!msgId) {
                return;
            }
...
exports.Room = Room;
function wechatifyRoom(wechaty) {
...
exports.wechatifyRoom = wechatifyRoom;
