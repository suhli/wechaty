 ...
 *
 */
const mod_1 = require("../src/mod"); // from 'wechaty'
...
function onScan(qrcode, status) {
    if (status === mod_1.ScanStatus.Waiting || status === mod_1.ScanStatus.Timeout) {
        qrcode_terminal_1.generate(qrcode);
        const qrcodeImageUrl = [
            'https://wechaty.js.org/qrcode/',
            encodeURIComponent(qrcode),
        ].join('');
        console.info('onScan: %s(%s) - %s', mod_1.ScanStatus[status], status, qrcodeImageUrl);
    }
    else {
        console.info('onScan: %s(%s)', mod_1.ScanStatus[status], status);
    }
...
async function onMessage(msg) {
     ...
     */
    const fileBox = mod_1.FileBox.fromUrl('https://wechaty.github.io/wechaty/images/bot-qr-code.png');
